...
export class BetService {
  static async placeBet(userId: string, input: PlaceBetInput): Promise<Bet> {
  ...
  static async getBetById(betId: string, userId: string): Promise<Bet> {
  ...
  static async settleBet(
    betId: string,
    result: 'won' | 'lost' | 'pushed' | 'cancelled'
  ): Promise<Bet> {
    const client = await getClient();
    ...
    try {
      ...
      const settledBet: Bet = updatedBet.rows[0];
      let transactionAmount = 0;
      let transactionType: 'bet_won' | 'bet_lost' | 'bet_refund' = 'bet_lost';
      let description = '';
  ...
  static async cancelBet(betId: string, userId: string): Promise<Bet> {
    const client = await getClient();
