...
import { OddsAPIGame, FormattedGame, AppError } from '../types';

export class OddsService {
  private static readonly API_KEY = process.env.ODDS_API_KEY;
  private static readonly BASE_URL = process.env.ODDS_API_BASE_URL || 'https://api.the-odds-api.com/v4';
  private static readonly DEFAULT_BOOKMAKER = 'draftkings'; // Preferred bookmaker
  ...
  static async getOdds(sportKey: string, markets: string = 'h2h,spreads,totals') {
    try {
      if (!this.API_KEY) {
  ...
  static async getEventOdds(sportKey: string, eventId: string) {
    try {
      if (!this.API_KEY) {
        throw new AppError('Odds API key not configured', 500);
      }
  ...
  private static formatGame(game: OddsAPIGame): FormattedGame {
  ...
  static calculatePotentialWin(odds: number, stake: number): number {
  ...
  static calculateProfit(odds: number, stake: number): number {
